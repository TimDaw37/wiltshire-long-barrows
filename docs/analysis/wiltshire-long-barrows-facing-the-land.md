...
The short answer is **no** — not as a shared design, and not in the way folklore likes. They do not all run along the ridges either. What they do, mostly, is spread.

You can look at every axis on the map:
...
Clive Ruggles spent a career insisting that prehistoric “astronomy” has to survive a proper test, not a pretty sunrise. His reading of the Wiltshire chalk — and his demolition of Aubrey Burl’s lunar-arc claim for Salisbury Plain long barrows — is still the honest baseline: **no clear common celestial alignment**, and the land matters.
...
Around Stonehenge, Historic England’s landscape surveys — Bax, Bowden, Soutar, Field, Barber and colleagues — drew the mounds that are actually there, not the polygons on a scheduling map. Dave Roberts and co-authors then put twenty-one of those WHS long barrows into *Internet Archaeology* 47: plans, dates, and a calm conclusion that **local topography is the key to alignment**, with a note that the more easterly end of many mounds seems to have mattered. They were generous enough to publish the table.

Timothy Darvill, Corcoran, Ashbee, Kinnes, Piggott: the catalogues and the Cotswold–Severn distinction. Environment Agency LiDAR, Historic England’s NHLE, the Wiltshire HER extracts that David Wheatley’s Zenodo recreation opened for reuse. We are adding a county-wide, chip-checked axis set on top of that, not instead of it.

If this paper is useful, it is because those people already did the hard looking.

---

## What was measured

A long barrow has a long axis: the line of the mound, not the way a façade or forecourt “faces”. That line has two ends. We treat it as **undirected** — a bearing between 0° and 180° from north. Midsummer sunrise and midwinter sunset are the same line once you fold the compass that way. That alone should make anyone slow down before saying “it faces the solstice”.
...
![Rose of 86 long axes](fig-01-rose-display.svg)
...
A weak east–west hint remains if you look only at HER-certain sites. It is not a solstice, and it is not a tight cluster. It is the sort of smear you would expect if people sometimes built along a down, sometimes across a spur, and often just along the mound they had room to raise.

![Histogram of axes](fig-02-histogram-10deg.svg)
...
If you score each barrow against *whichever* solstice is closer, the match looks better — until you give a random set of axes the same two targets. Chance is then just as close. That is the whole of the solar result.
...
We fitted a plane to the Environment Agency terrain in a ring around each mound (close enough to be “here”, far enough out that the barrow itself is not the hill). The long axis sits nearer the contour than a coin-toss would put it. It prefers the ridge line to the fall line. **It does not glue itself to the ridge.** Half the sample is still more than thirty degrees off the local ground.

That is exactly the picture Field, McOmish, Ruggles and Roberts described in words: careful siting, no single recipe.

![Axes versus local ridge](fig-12-axis-vs-ridge.svg)
...
![West and East Kennet](fig-16-kennet-pair.svg)
...
So the old description of West Kennet is fair at the scale of the valley. A sceptic who says “it follows the ridge” is looking at the same ground we are. They cannot then say the Kennet long barrows *as a class* follow the ridge: East Kennet is the neighbour that doesn’t.

What the county-wide computer test measures is blunter: the best-fit *tilt* of a ring of ground around each mound, so the barrow itself is not counted as the hill. A ridge falls both ways. Fit a plane to it and the steeper or broader flank wins. At West Kennet that flank is the rise westward onto the downs, so the machine reports the axis as “across the local slope.” At East Kennet the same test does the opposite: the ring is the north-east hillside, so the machine reports the axis as *along* the local tilt — the scarp, not the watershed. Neither report is the named ridge. Believe the contours.
...
That is the point of doing the work this way. The new county set is not a rival catalogue. It is a way to take their standard of looking — mound, not paperwork — out across Wiltshire, including the unscheduled and the almost-ploughed.
...
On the [gazetteer](https://timdaw37.github.io/wiltshire-long-barrows/), gold markers are HER-certain, grey are possible. **Rim arrows are the long axis where we could see one.** A plain circle means we could not, and we have not borrowed a polygon to fake it.
...
Figures, tables and the full statistical note live with the technical companion: [wiltshire-long-barrow-orientations.md](wiltshire-long-barrow-orientations.md).
